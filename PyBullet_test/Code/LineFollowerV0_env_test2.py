...
class LineFollowerV0(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.client.resetBasePositionAndOrientation(self.car, (-3.536647007992482, 0.4236722300659929, 0.09807812190507681),
                                                    (-0.0014877329980157537, -0.0016113340695280331, -0.676497446239607, 0.7364417122110434))
        ...
        for joint_index in self.wheel_indices:
            self.client.setJointMotorControl2(self.car, joint_index, self.client.VELOCITY_CONTROL,
                                              targetVelocity=4)
        self.angle = 0
    ...
    def step(self, action):
        ...
        closest_waypoint_distance, passed = self.waypoint_monitor.step_monitor(threshold=0.15)
        ...
        terminated_bad = (closest_waypoint_distance > 2)
        ...
        reward = (passed * 2 - closest_waypoint_distance ** 2 +
                  (~(terminated_good)) * (truncated or terminated_bad) * passed_ratio * 10 +
                  terminated_good * 10 - 0.1 - terminated_bad * 5)
...
eval_callback = EvalCallback(eval_vec_env, eval_freq=6000,
