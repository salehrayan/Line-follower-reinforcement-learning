...
class LineFollowerV0(gym.Env):
    ...
    def step(self, action):
        ...
        user_angle = action[1] * 1.2
        for joint_index in self.wheel_indices:
            self.client.setJointMotorControl2(self.car, joint_index, self.client.VELOCITY_CONTROL, targetVelocity=user_throttle)
        for joint_index in self.hinge_indices:
            self.client.setJointMotorControl2(self.car, joint_index, self.client.POSITION_CONTROL, targetPosition=user_angle)
        ...
        terminated_bad = (abs(car_pos[0]) > 9 ) or (abs(car_pos[1]) > 9)
        ...
        reward = (passed * 1 - np.clip(closest_waypoint_distance, 0, 2) +
                  (~(terminated_good or terminated_bad)) * truncated * passed_ratio * 10 +
                  terminated_good * 10 - 0.1 - terminated_bad * 5 + user_throttle / 20)
...
vec_env = DummyVecEnv([lambda: env, lambda: env2, lambda: env3, lambda: env4])
