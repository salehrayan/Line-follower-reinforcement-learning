...
class ReplayMemory:
    ...
    def sample(self, batch_size):
        ...
        return np.array(batch[0], dtype=np.float32), torch.tensor(batch[1], dtype=torch.float64), np.array(batch[2], dtype=np.float32), np.array(batch[3], dtype=np.bool_), np.array(batch[4], dtype=np.float32)

    def can_sample(self, batch_size):
        return len(self.memory) >= batch_size * 1
...
class Agent_DQN():
    ...
    def __init__(self, save_path, load_path, num_episodes, max_steps,
                 learning_rate, batch_size, gamma, clip_grad_norm, baseline):
    ...
    def train(self):
        ...
        for episode in range(1, self.num_episodes + 1):
            ...
            while True:
                ...
                self.memory.insert([state, action.to(device), reward, done, next_state])

                if self.memory.can_sample(self.batch_size):
                    state_b, action_b, reward_b, done_b, next_state_b = self.memory.sample(self.batch_size)
                    qsa_b = q_network(torch.from_numpy((state_b-np.mean(state_b,axis=1, keepdims=True))/(np.std(state_b,axis=1, keepdims=True)+0.01)).to(device)).gather(1, action_b.clone().type(torch.int64).detach().to(device).unsqueeze(1))

                    next_qsa_b = target_q_network(torch.from_numpy((next_state_b-np.mean(next_state_b,axis=1, keepdims=True))/(np.std(next_state_b,axis=1, keepdims=True)+0.01)).to(device))
                    next_qsa_b = torch.max(next_qsa_b, dim=-1, keepdim=True)[0]

                    target_b = torch.tensor(reward_b).to(device)+ torch.tensor(~done_b).to(device) * self.gamma * next_qsa_b.squeeze(1)
                    loss = torch.nn.functional.mse_loss(qsa_b.squeeze(1).float(), target_b)
...
save_path = './Deep_Q_learning_results/best_weights_using_gs_fc_128_100ms_ExperienceReplay.pt'
load_path = './Deep_Q_learning_results/best_weights_using_gs_fc_128_100ms_ExperienceReplay.pt'
train_mode = False
