...
class Environment(Supervisor):
    ...
    def get_reward(self):
        ...
        reward = ~(gs_values[0]> 450 and gs_values[1]> 450 and gs_values[2]> 450) * \
                 ...
                 (gs_values[0] > 450 and gs_values[1] > 450 and gs_values[2] > 450) * -500 + \
                 (left_motor_speed>0.5)*left_motor_speed**2 + (right_motor_speed>0.5)*right_motor_speed**3 \
                 - abs(left_motor_speed-right_motor_speed)*self.rotation_punishment_factor +\
            collision * -50000 + (left_motor_speed < 0.5 or right_motor_speed < 0.5) * -150
        # reward = ~(gs_values[0] > 450 and gs_values[1] > 450 and gs_values[2] > 450) * \
        #          ((gs_values[1] > gs_values[0] and gs_values[1] > gs_values[2]) * 70 + \
        #           (gs_values[0] > gs_values[1] and gs_values[0] > gs_values[2]) * 15 + \
        #           (gs_values[2] > gs_values[1] and gs_values[2] > gs_values[0]) * 15) + \
    ...
    def apply_action(self, action):
        ...
        match action:
            case 0:
                pass
            case 1:
                self.left_motor.setVelocity(left_motor_speed - 2)
            case 2:
                self.right_motor.setVelocity(right_motor_speed - 2)
            case 3:
