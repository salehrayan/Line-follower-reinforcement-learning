...
class NETWORK(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = nn.functional.avg_pool2d(x, 56).squeeze(2).squeeze(2)
        x = self.fc1(x)
    ...
    def train(self):
        ...
        for episode in range(1, self.num_episodes + 1):
            ...
            while True:
                ...
                action = e_greedy_policy(torch.from_numpy(state).to(device).unsqueeze(0)/255.0, epsilon=0.15)
                ...
                loss = self.compute_loss((state)/255.0, action,
                                         reward, (next_state)/255.0, done)
            ...
            while not done:
                ...
                if torch.rand(1) < 0.05:
                    ...
                    action = torch.argmax(q_network(
                        torch.from_numpy(state).to(device).unsqueeze(0)/255.0),
                                          dim=-1)
...
save_path = './Deep_Q_learning_results/best_weights_using_cam_100ms.pt'
load_path = './Deep_Q_learning_results/best_weights_using_cam_100ms.pt'
train_mode = True
